...
def black_scholes_fx_put(S0: float, K: float, rd: float, rf: float, vol: float, T: float) -> float:
    """
    Prices a European FX put option via the Garman-Kohlhagen (1983) model.
    ...
    d1 = (np.log(S0 / K) + (rd - rf + 0.5 * vol ** 2) * T) / (vol * np.sqrt(T))
    d2 = d1 - vol * np.sqrt(T)
    return K * np.exp(-rd * T) * norm.cdf(-d2) - S0 * np.exp(-rf * T) * norm.cdf(-d1)